...
function getPolynom(...coefficients) {
  ...
  return (x) => {
    let sum = 0;
    const power = coefficients.length - 1;
...
function memoize(func) {
  ...
  return function memoizeArgs(...args) {
    const key = JSON.stringify(args);

    if (!cache[key]) {
      cache[key] = func.apply(args, this);
    }
    return cache[key];
  };
}
...
function retry(func, attempts) {
  return function attemptsCounter() {
    let lastError;
...
function logger(func, logFunc) {
  return function funcArgs(...args) {
    const argsString = args.map((a) => JSON.stringify(a));
...
function partialUsingArguments(fn, ...args1) {
  return function restArgsFunc(...args2) {
